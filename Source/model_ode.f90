  ...
  implicit none
    ...
    do while (mp_time < t_intv)
       ...
       if (soln_method == 'dvr') then
          ...
          do l = 2,n
             RHS(l-1) = sqrt(dvr_weights(l))*(mp_points(l)-alpha)*complex(cos(theta1), sin(theta1))*mp_psi
          end do
          ...
          do q = 2,n
             ...
             mp_iterate(q) = complex(cos(-theta), sin(-theta))*(RHS(q-1)/sqrt(dvr_weights(q)) &
                  + complex(cos(theta1), sin(theta1))*mp_psi)
          end do
          ...
          do k = 2,n
             ...
             mp_inhomogeneity(k) = complex(cos(-theta), sin(-theta))*mp_psi
          end do
          ...
          do while ((.not. mp_converged) .and. (iteration_number < it_cap))
             ...
             do l = 2,n
                do i = 1,n
                   ...
                   y(i) = complex(cos(-theta), sin(-theta))*(mp_points(i) - alpha)*mp_phi(i)
          ...
          do j = 2,n
             ...
             mp_inhomogeneity(j) = complex(cos(-theta), sin(-theta))*mp_psi
          end do
             ...
             if (it_type == 'gauss_seidel') then
                do k = 2,n
                   ...
                   do m = 1,k-1
                      ...
                      b = b - ii*comp_wt(m,k-1)*complex(cos(-theta), sin(-theta))*mp_vpsi(m)
                   end do
                ...
                do k = 2,n
                   ...
                   do m = 1,n
                      theta = alpha*(mp_points(k) - mp_points(m))
                      mp_iterate(k) = mp_iterate(k) - ii*comp_wt(m,k-1)*complex(cos(-theta), sin(-theta))*mp_vpsi(m)
                   end do
       ...
       do r = 1,n
          exact_solution = complex(cos(-0.50d0*mp_points(r)*mp_points(r)), sin(-0.50d0*mp_points(r)*mp_points(r)))
